...
Date.prototype.print = function (str, dateType, useLangNumbers) {
	...
	var hr = this.getLocalHour(true, dateType);
	//End : Nassim
	var pm = (hr >= 12);
	var ir = (pm) ? (hr - 12) : hr;
    ...
    var min = this.getLocalMinute(true, dateType);
	...
	s["%H"] = hr;//(hr < 10) ? ("0" + hr) : hr; // hour, range 00 to 23 (24h format)
	s["%I"] = (ir < 10) ? ("0" + ir) : ir; // hour, range 01 to 12 (12h format)
	s["%j"] = (dy < 100) ? ((dy < 10) ? ("00" + dy) : ("0" + dy)) : dy; // day of the year (range 001 to 366)
	s["%k"] = hr;		// hour, range 0 to 23 (24h format)
	s["%l"] = ir;		// hour, range 1 to 12 (12h format)
	s["%m"] = (m < 9) ? ("0" + (1+m)) : (1+m); // month, range 01 to 12
	s["%M"] = min;//(min < 10) ? ("0" + min) : min; // minute, range 00 to 59
	s["%n"] = "\n";		// a newline character
...
Date.prototype.getLocalHour = function(UTC, dateType) {
   var hourTxt = '';
   for(i=0; (span = document.getElementsByTagName("span")[i]); i++) {
       if (span.getAttribute('class') == 'hour') {
            hourTxt = span.innerHTML;
       }
   }
   return hourTxt;
}

Date.prototype.getLocalMinute = function(UTC, dateType) {
   var minTxt = '';
   for(i=0; (span = document.getElementsByTagName("span")[i]); i++) {
       if (span.getAttribute('class') == 'minute') {
           minTxt = span.innerHTML;
       }
   }
   return minTxt;
}
