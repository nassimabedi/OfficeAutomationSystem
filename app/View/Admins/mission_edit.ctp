<div class="users form" dir="" ng-controller="missionEditCrtl">
<form name="form"  ng-submit="submitForm(form.$valid)" novalidate>  
    <fieldset dir="rtl">
        <legend style="padding-bottom:1%">ویرایش ماموریت</legend>
        <style type="text/css">
        .addtable  { 
          border-spacing: 5px;
         width:100%;
    ...
    <input type="hidden" name="mission_id" value="<?= $data['Missions']['id'] ?>" ng-model="mission_id">
        <table class="addtable">
      <tr>
        <td colspan="2" align="center"><span style="color:blue" name="messeg" ng-model="messeg">{{message}}</span></td>
      </tr>
      <tr>
          <td class="row-cus" width="12%">کاربران*:</td>
          <td class="row-cus">
      ...
      <tr>                
        ...
        <div class="col-md-3 pull-right" style="padding: 30px 10px 10px 10px;">
            <p class="input-group pull-right">
              <input type="text" class="form-control"  datepicker-popup-persian="{{format}}" name="date" ng-model="date" is-open="persianIsOpen"  datepicker-options="dateOptions" date-disabled="disabled(date, mode)"  close-text="بسته" required value="<?=$data['Missions']['date'] ?>" />
      ...
      <tr >
          ...
          <td><input type="text" name="purpose" ng-model="purpose" required><span class="help-inline" ng-show="submitted && form.purpose.$error.required" style="color:red;padding-right:10px">لطفا فیلد به جهت را وارد نمایید</span></td>
      </tr>
            <tr >
          ...
          <div class="row">
            <div class="col-md-3 pull-right" style="padding: 30px 10px 10px 10px;">
                <p class="input-group pull-right">
                  <input type="text" class="form-control"  datepicker-popup-persian="{{format}}" name="start_date" ng-model="start_date" is-open="persianIsOpenStartDate"  datepicker-options="dateOptions" date-disabled="disabled(date, mode)" ng-required="true" close-text="بسته" required />
                   <span class="input-group-btn">
        ...
        <td >
          <div class="row">
            <div class="col-md-3 pull-right" style="padding: 30px 10px 10px 10px;">
                <p class="input-group pull-right">
                  <input type="text" class="form-control"  datepicker-popup-persian="{{format}}" name="end_date" ng-model="end_date" is-open="persianIsOpenEndDate"  datepicker-options="dateOptions" date-disabled="disabled(date, mode)" ng-required="true" close-text="بسته" required />
                   <span class="input-group-btn">
      ...
      <tr>
          ...
          <td><textarea name="description" ng-model="description" cols="30">{{description}}</textarea></td>
      </tr>
      <tr>       
        <td colspan="2" class="row-cus">
          ...
          <button ng-click="uploadFile()" class="btn btn-default"> <i class="glyphicon glyphicon-upload"></i>upload File</button> <a ng-click="download_file()"><span ng-show="mission_file_dl">{{mission_file_dl}}</span></a>           
        </div>

        </td>
      </tr>
      <tr>
          <td class="row-cus">تایید:</td>
          <td><input type="checkbox" ng-model="approve"
                 ng-true-value="1" ng-false-value="0" ng-checked="{{approve}}"></td>
      </tr>
      <tr>
